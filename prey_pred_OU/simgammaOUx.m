function simregx = simgammaOUx(a_x, b_x, xo, startt, endt, dt, lambda1)

%simulaating Poisson process
N = simpoipro1(a_x*lambda1, startt, endt, 0, 1); 
...
% Simulate the OU process 
    ...
    for j = n(k-1)+1 : n(k)
        sumx = sumx - log(rand(1))/b_x;
    end
    
    x(k)=(1-lambda1*dt)*x(k-1)+sumx;
end
